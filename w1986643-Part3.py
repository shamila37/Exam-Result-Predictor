...
while student :
    ...
    if 120 == credits_at_pass :
        print("Progress")
        data_file.write("Progress - " + str(credits_at_pass) + "," + str(credits_at_defer) +
                        "," + str(credits_at_fail) + "\n")
        progress = progress + 1
    elif 100 == credits_at_pass :
        print("Progress (module trailer)")
        data_file.write("Progress (module trailer) - " + str(credits_at_pass) + "," +
                        str(credits_at_defer) + "," + str(credits_at_fail) + "\n")
        module_trailer = module_trailer + 1
    elif 60 <= ( credits_at_pass + credits_at_defer ) :
        print("Do not Progress – module retriever")
        data_file.write("Do not Progress – module retriever - " + str(credits_at_pass) +
                        "," + str(credits_at_defer) + "," + str(credits_at_fail) + "\n")
        module_retriever = module_retriever + 1
    else :
        print("Exclude")
        data_file.write("Exclude - " + str(credits_at_pass) + "," + str(credits_at_defer) +
                        "," + str(credits_at_fail) + "\n")
        exclude = exclude + 1
